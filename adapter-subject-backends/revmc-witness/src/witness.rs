...
use crate::strict_db::{StrictDb, StrictDbError};
...
use reth_trie_sparse::{
    ...
    SparseTrie as SparseTrieTrait, TrieNodeEpoch,
};
use revm::{
...
impl WitnessDb {
    ...
    pub fn into_verified_post_state_root(
        ...
        let mut post_state = HashedPostState::from_bundle_state::<KeccakKeyHasher>(bundle.state());
        // Reth 42's HashedPostState conversion copies present slots but does not project the
        // BundleAccount destruction bit. Preserve it explicitly so destroy+recreate starts from
        // an authenticated empty storage trie instead of requiring obsolete slot proofs.
        ...
        for hashed_address in &changed_addresses {
            ...
            let storage_root = if let Some(storage) = post_state.storages.get(hashed_address) {
                ...
                match sparse_update {
                    Ok(()) => storage_trie
                        .as_mut()
                        .and_then(|trie| trie.root(TrieNodeEpoch::new(self.target_header.number)))
        ...
        let actual = match sparse_update {
            Ok(()) => {
                self.sparse
                    .root(TrieNodeEpoch::new(self.target_header.number))
